...
const api: ProjectDApi = {
  ...
  getSuggestionSuppressionHistory: () => ipcRenderer.invoke(IPC_CHANNELS.SUGGESTIONS_GET_SUPPRESSION_HISTORY),
  ...
  prepareLivePhotoImport: () => ipcRenderer.invoke(IPC_CHANNELS.WALLPAPER_PREPARE_LIVE_PHOTO),
  confirmLivePhotoImport: (token) => ipcRenderer.invoke(IPC_CHANNELS.WALLPAPER_CONFIRM_LIVE_PHOTO, token),
  cancelLivePhotoImport: (token) => ipcRenderer.invoke(IPC_CHANNELS.WALLPAPER_CANCEL_LIVE_PHOTO, token),
  importGeneratedWallpaper: (dataUrl, label) => ipcRenderer.invoke(IPC_CHANNELS.WALLPAPER_IMPORT_GENERATED, dataUrl, label),
  ...
  draftPetVisualProfile: (request) => ipcRenderer.invoke(IPC_CHANNELS.AI_PET_VISUAL_DRAFT, request),
  savePetVisualProfile: (characterId, profile) => ipcRenderer.invoke(IPC_CHANNELS.AI_PET_VISUAL_SAVE, characterId, profile),
  exportAllData: () => ipcRenderer.invoke(IPC_CHANNELS.PRIVACY_EXPORT_DATA),
  clearRuntimeCache: () => ipcRenderer.invoke(IPC_CHANNELS.PRIVACY_CLEAR_CACHE),
