...
export class DatabaseService {
  ...
  updateContainerPosition(containerId: number, x: number, y: number, width: number, height: number, isCollapsed?: boolean): void {
    const params: Array<number> = [
      x,
      y,
      Math.max(112, Math.min(800, width)),
  ...
  applyLayout(layoutId: number, workAreaWidth = 1920, workAreaHeight = 1080): void {
    ...
    const gap = requestedColumns >= 8 ? 8 : requestedColumns >= 6 ? 10 : 16;
    const left = 24;
    const top = 96;
    const bottom = 24;
  ...
  private seedDefaults(): void {
    ...
    try {
      ...
      this.seedState("theme_mode", "dark");
      this.seedState("wallpaper_save_original", "false");
