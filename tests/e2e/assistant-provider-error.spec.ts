...
test("assistant distinguishes a provider HTTP error from a timeout and keeps local fallback usable", async () => {
  const server = createServer((_request, response) => {
    response.writeHead(503, { "Content-Type": "application/json" });
    response.end(JSON.stringify({ error: { message: "qa provider unavailable" } }));
  });
  ...
  const testApp = await launchProjectD("assistant-provider-error", {
    env: {
      PROJECTD_QA_IDLE: "0",
  ...
  try {
    ...
    await testApp.window.evaluate((apiEndpoint) => window.projectD.updateSettings({
      ai: { enabled: true, provider: "openai-compatible", apiEndpoint, model: "qa-http-error" }
    ...
    await input.fill("请回答一个服务异常降级测试");
    await input.press("Enter");

    const status = testApp.window.locator(".assistant-surface .chat-status");
    await expect(status).toHaveAttribute("data-tone", "error");
    await expect(status).toContainText("云端服务不可用");
    await expect(status).toContainText("本地降级");
    await expect(input).toHaveValue("");
    await expect(testApp.window.locator(".assistant-surface .chat-history")).toContainText("收到");
    await testApp.window.screenshot({ path: path.join(outputDir, "02-provider-error.png"), fullPage: true });
  } finally {
