...
const { wallpaperRenderProfile, wallpaperRenderScale } = require("../dist/shared/wallpaper-render-scale.js");
...
test("runtime battery saver maps to the wallpaper battery budget", () => {
  assert.equal(wallpaperRenderProfile("quality"), "quality");
  assert.equal(wallpaperRenderProfile("balanced"), "balanced");
  assert.equal(wallpaperRenderProfile("batterySaver"), "battery-saver");
});
