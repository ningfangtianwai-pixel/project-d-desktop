...
const petCharacterDir = path.join(root, "public", "pet", "characters");
...
for (const directory of [wallpaperDir, petCharacterDir]) {
  for (const file of fs.readdirSync(directory).sort()) {
    const relative = path.normalize(path.relative(root, path.join(directory, file)));
