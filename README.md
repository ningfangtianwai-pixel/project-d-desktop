...
> 当前产品目标：Project D V6.0 沉浸式 AI 桌面空间。程序内部版本仍为 `0.3.0-dev.0`，当前处于 V6.0 重构前稳定基线，仅适合开发和受控测试，不代表已经通过公开发布、商用素材、代码签名、真实硬件矩阵或法律审核门禁。
...
V6.0 当前范围不包含生产账号、订单、支付、会员、云同步或 Project D 云端遥测服务。仓库中的历史计划、未来领域模型或测试骨架不表示对应服务已经上线。

V6.0 的正式执行基线见 [沉浸式 AI 桌面空间重构执行计划](docs/ProjectD_V6.0_沉浸式AI桌面空间重构执行计划.md)。V4.0/V5.1 文档与标签作为历史依据保留。
